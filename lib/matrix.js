...
/**
 * Replace row i, col j element in Matrix with value 
 * @param {number} i - Index of row to replace
 * @param {number} j - Index of column to replace 
 */
Matrix.prototype.set = function (i, j, val) {
  assert(i >= 0 &&  j >= 0 && i <= this.rows - 1 && j <= this.cols - 1,
    'Matrix row and col given does not exist');
    this.elements[i * this.cols + j] = val;
    return this;
};

/**
 * Swap positions of row i and j in Matrix
 * @param {number} i - Index of first row in swap
 * @param {number} j - Index of second row in swap
 */
Matrix.prototype.swap = function(idxOne, idxTwo) {
	assert(idxOne >= 0 && 
		idxTwo >= 0 && 
		idxOne <= this.rows - 1 && 
		idxTwo <= this.rows - 1,
    'Matrix row indices do not exist');
	let arrayMat = this.asArray(); 
    let rowOneCopy = arrayMat[idxOne];
    let rowTwoCopy = arrayMat[idxTwo]; 
    arrayMat[idxOne] = rowTwoCopy; 
    arrayMat[idxTwo] = rowOneCopy; 
    this.elements = traversable.flatten(arrayMat); 
    return this; 
  };
...
Matrix.prototype.colVectors = function() {
	...
	let colGroups = traversable.group(matrixT.elements)(this.rows);
...
/**
 * Performs Gaussian elimination to return a
 * new new matrix in upper triangular form.
 */
